...
const Auth = () => {
  const [accessToken, setAccessToken] = useState(localStorage.getItem('instagram_access_token'));
  ...
  useEffect(() => {
    ...
    const getAccessToken = async (authorizationCode) => {
      ...
      try {
        ...
        // Eliminar el parámetro 'code' de la URL
    ...
    <div>
      {accessToken ? (
        <>
          ...
          {userProfile && (
            <div>
              <p>ID de Usuario: {userProfile.id}</p>
